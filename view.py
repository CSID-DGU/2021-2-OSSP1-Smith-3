...
def reconstruct():
    ...
    while needReconstruct():
        ...
        if len(script_table[s_idx]) > len(voice_table[v_idx]):
            ...
            while diff > 0:
                if len(voice_table[v_idx+1]) >= diff:
                    voice_table[v_idx] = voice_table[v_idx] + \
                        voice_table[v_idx+1][0:diff]
        ...
        # voice가 덜 쪼개짐 ex) script[idx]= '해외' voice[idx] = '해외여행'
            ...
            voice_table.insert(
                v_idx+1, voice_table[v_idx][:len(script_table[s_idx])])
            voice_table.insert(
                v_idx+2, voice_table[v_idx][len(script_table[s_idx]):])
