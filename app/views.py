...
def colloquial(request):
    return render(request, 'app/colloquial.html')
...
def greetings(request):
    return render(request, 'app/greetings.html')
...
def tonguetwister(request):
    return render(request, 'app/tonguetwister.html')
                    ...
                    while len(one_sentence) != 0:
                        ...
                        for i in index_arr:  # 앞 뒤 여섯 문장까지 검사
                            ...
                            if i in trueSentenceIndex or i in yellowSentenceIndex:  # 이미 말한 문장에 있으면 다음 문장 검사
                                continue
                            # 맞는 문장 찾았다면 그만 검사
                            if super_compare(i, voice_table[0], one_sentence):
                                flag = True
                                if lastTrueIndex > i:
                                    script_index = i+1
...
def subject_compare(script_sentence, voice_sentence):
    if script_sentence[0]:
        if voice_sentence[0]:
            if find_N(script_sentence[0]) == find_N(voice_sentence[0]):
                return True
            else:
                return False
        else:
...
def object_compare(script_sentence, voice_sentence):
    if script_sentence[1]:
        if voice_sentence[1]:
            if find_N(script_sentence[1]) == find_N(voice_sentence[1]):
                return True
            else:
                return False
        else:
...
def adnominal_noun_compare(script_index, one_sentence, voice_sentence):
    ...
    for i in range(len(script_modifier_table)):
        ...
        if not found_noun:                          # 같은 명사 못 찾았을 시 False
            return False
    if adnominal_flag:
        return True                  # 같은 명사의 수식어가 같으면 True
    else:
        return False                              # 같은 명사의 수식어가 다르면 False
    return False
...
def change_active_passive(script_sentence_component, voice_sentence_component):
    ...
    if script_sentence_component[0] and voice_sentence_component[4] and script_sentence_component[1] and \
        ...
        # script의 주어가 voice 부사어에 있나 확인
        for i in range(0, len(voice_sentence_component[4])):
            if (script_sentence_component[0][0][0] == voice_sentence_component[4][i][0]):
                subject_equal_adverb = True
        # script 목적어와 voice 주어가 같나 확인
        if script_sentence_component[1][0][0] == voice_sentence_component[0][0][0]:
            object_equal_subject = True

        # script의 본 동사 찾기
        for i in range(0, len(script_sentence_component[2])):
    ...
    if script_sentence_component[0] and voice_sentence_component[1] and script_sentence_component[4] and \
            voice_sentence_component[0] \
            and script_sentence_component[2] and voice_sentence_component[2]:
        # script 주어와 voice 목적어가 같은가
        if script_sentence_component[0][0][0] == voice_sentence_component[1][0][0]:
            subject_equal_object = True
        for i in range(0, len(script_sentence_component[4])):
            # script 부사어와 voice 주어가 같은가
            if script_sentence_component[4][i][0] == voice_sentence_component[0][0][0]:
...
# '못' 뒤에 띄어쓰기 추가하는 함수 : '못'을 기준으로 split한 후, 각 요소 사이에 '못+공백'을 추가하여 합침.
def add_space_after_mot(input_string):
...
# 문장의 마지막인지 판단 : EF[종결어미] 이거나 EC(연결어미)로 분석된 마지막 요소
...
def each_sentence_division(script_string_array):  # 한 문장 단위로 끊어서 분석하는 함수
    ...
    # 문장 단위로 끊어져 저장되어 있는 배열에서 한 문장씩 (each) 형태소 분석
    for each in script_string_array:
        complete_sentence = []
        sentence = str(each)
        ...
        one_line_temp = make_element_table(
            complete_sentence, script_string_array[count])
        element_table = np.append(element_table, np.array(
            [one_line_temp], dtype=list), axis=0)
        modifier_table.append(make_modifier_table(
            script_string_array[count], complete_sentence))
        count += 1
...
def sentence_division(input_string):
    ...
    # ex) [('안녕', 'NNG'), ('하', 'XSV'), ('세요', 'EP+EF')]
    mecab_result = mecab.pos(input_string)
    ...
    for i in range(len(mecab_result)):
        if is_sentence_End(mecab_result[i]):  # 문장의 마지막인지 판단
            sentence = []
            # 한 문장 내의 첫번째 요소부터 마지막 요소까지 저장.
            for j in range(string_start, i + 1):
...
def make_modifier_table(input_string, mecab_string):
    ...
    for i in range(len(temp_string)):
        ...
        if temp_string[i][1].find('ETM') != -1:  # 관형형 전성어미를 통해 관형어를 찾음
            # 관형형 전성어미가 다른 형태소에 포함되어 나오는 경우(ex.('못생긴', 'VA+ETM'))
            if temp_string[i][1].find('+') != -1:
                ...
                if temp_string[i - 1] == temp_string[
                    ...
                    # 만약 관형형 전성어미 뒤에서 문장이 잘린 경우 테이블에 넣으면 안됨
                    if temp_string[i] != temp_string[len(temp_string) - 1]:
                ...
                # 관형형 전성어미가 붙어서 한번에 나오는 경우 그 단어만 관형어 list에 추가(ex.('아름다운', 'VA+ETM'))
                else:
                    # 만약 관형형 전성어미 뒤에서 문장이 잘린 경우 테이블에 넣으면 안됨
                    if temp_string[i] != temp_string[len(temp_string) - 1]:
                        modifier_line_two[0].extend(temp_string[i])
            ...
            # 관형형 전성어미가 다른 형태소에 포함되지 않고 나오는 경우(ex.('작', 'VA'), ('은', 'ETM'))
            else:
                # 만약 관형형 전성어미 뒤에서 문장이 잘린 경우 테이블에 넣으면 안됨
                if temp_string[i] != temp_string[len(temp_string) - 1]:
                    modifier_line[0].extend(temp_string[i - 1])
...
def find_s(sentence):
    ...
    for k in range(len(sentence)):  # 테이블에 저장된 한 문장 길이 동안
        if ((sentence[k][0] == '가' and sentence[k][1] == 'JKS') or (sentence[k][0] == '이' and sentence[k][1] == 'JKS')):
            # 뒤에 '되', '돼'가 오면 보어로 처리해야함
            do_jamo = j2hcj(h2j(sentence[k + 1][0]))
...
def find_verb(input_string):
    ...
    for j in range(0, len(input_string)):
        # 문장 요소에 V가 있다면
        if start_flag == -1 and is_have_tag('V', input_string[j]):
    ...
    if start_flag != -1:  # start_flag가 -1이면 서술어가 없다는 것
        if start_flag > 0:  # start flag가 0이라면 앞에 것 볼 필요X
            # start_flag 앞의 토큰이 명사라면
            if is_have_tag('NNG', input_string[start_flag - 1]):
...
# (체언 단독의 경우(ex.우연히 고향 친구를 만났다)와 체언의 자격을 가진 말 + 관형격 조사의 경우(ex.그는 웃기기의 천재다) 아직 처리 X)
def find_tubular(input_string, mecab_string):
    temp_string = mecab_string
    tubularArr = []

    for i in range(len(temp_string)):
        # 관형격 조사를 통해 관형어를 찾음. 관형격 조사와 그 앞의 단어는 관형어이므로 이를 list에 추가
        ...
        if temp_string[i][1].find('ETM') != -1:  # 관형형 전성어미를 통해 관형어를 찾음
            ...
            if temp_string[i][1].find('+') != -1:
                ...
                if temp_string[i - 1] == temp_string[
                        len(temp_string) - 1]:  # 여러 개의 관형형 전성어미가 나올 경우 띄어쓰기로 각각을 구분하기 때문에 맨 앞의 관형형 전성어미에 대한 예외처리
                    tubularArr.append(temp_string[i])
                # 관형형 전성어미가 다른 형태소와 합성되어 있으며 그 앞에 다른 형태소가 나오는 경우 관형형 전성어미의 앞의 단어도 list에 추가(ex.('깨끗', 'XR'), ('한', 'XSA+ETM'))
                elif input_string[inputIndex - 1] != ' ':
                    tubularArr.append(temp_string[i - 1])
                    tubularArr.append(temp_string[i])
                # 관형형 전성어미가 붙어서 한번에 나오는 경우 그 단어만 관형어 list에 추가(ex.('아름다운', 'VA+ETM'))
                else:
...
def find_tense(sentence):
    tense_table = [['past', ], ['present', ],
                   ['future', ]]  # 문자열과 시제를 함께 저장할 테이블
    ...
    for i in range(len(sentence)):
        ...
        if sentence[i][1].find('NNB') != -1 and sentence[i][0].find('것') != -1:
            ...
            # 종성이 있고, -ㄹ 것이 가 미래형으로 구분
            if len(do_jamo) > 2 and do_jamo[2] == 'ㄹ':
